...
export default class AircraftModel {
    ...
    constructor(options = {}) {
        ...
        */
        this.trueTransponderCode = 0;

        /**
         * used for aircraft-id alteration attack
         */
        this.trueId = 0;
        ...
        */
        ...
        /**
        * for attacking this specific aircraft
        */
        this.isAttackTarget = 0;
    ...
    * @param callsign {string}
    ...
    assignAttackValue() {
      ...
      var log_string ="no Response = "+rates["response"].toFixed(1)+"%,\n jumping = "+rates["jump"].toFixed(1)+"%,\n error = "+rates["falseInformation"].toFixed(1);
      log_string += "%,\n stand still = "+rates["standStill"].toFixed(1)+"%,\n modified trajectory = "+rates["trajectoryModification"].toFixed(1)+"%,\n transponder code alteration = ";
      log_string += rates["transponderCodeAlteration"].toFixed(1)+"%,\n id spoofing = "+rates["idSpoofing"].toFixed(1)+"%,\n message delay = "+rates["messageDelay"].toFixed(1)+"%\n  At:  100/"+this.amountOfAttack;
      console.log(log_string);

      const random = Math.floor(Math.random() * this.amountOfAttack);
      const threshold1 = rates["response"];
      const threshold2 = threshold1 + rates["jump"];
      const threshold3 = threshold2 + rates["falseInformation"];
      const threshold4 = threshold3 + rates["standStill"];
      const threshold5 = threshold4 + rates["trajectoryModification"];
      const threshold6 = threshold5 + rates["transponderCodeAlteration"];
      const threshold7 = threshold6 + rates["idSpoofing"];
      const threshold8 = threshold7 + rates["messageDelay"]


      if ((random < 100) && sum != 1 && this.amountOfAttack != 9999999){
          if (random < threshold1){ //non listener
              this.attackType = rarities["response"].attack;
              GameController.responsers++;
          } else if ((random >= threshold1) && (random < threshold2)){ //jumper
              this.attackType = rarities["jump"].attack;
              GameController.jumpers++;
          } else if (random >= threshold2 && random < threshold3 ){ //false information
              this.attackType = rarities["falseInformation"].attack;
              GameController.errorers++;
          } else if (random >= threshold3 && random < threshold4){ //stopper
          ...
          } else if (random >= threshold6 && random < threshold7){ // id spoofing
            this.attackType = rarities["idSpoofing"].attack;
            this.falseAircraftIdAttack();
            GameController.spoofers++;
          } else if (random >= threshold7 && random < threshold8) { // message delay
            this.messageDelay = GameController.messageDelay;
            if (this.messageDelay != 0){
                this.attackType = rarities["messageDelay"].attack;
                GameController.delayers++;
            }
          }
      } else {
        this.attackType = 0;
    ...
    */
    ...
    update(endTime) {
      ...
      if(endTime == 1 || ((this.pastAmountofAttack != this.amountOfAttack || tempNeedUpdate != this.needUpdate) && !this.isFlooding && this.attackType != 8 && !this.isAttackTarget)){
    ...
    startAttackForTarget(){
        switch(this.attackType){
            ...
            case 7:
                ...
                break;
            case 9:
                this.messageDelay = GameController.dRarity;
                GameController.delayers++;
                break;
        }
    }
    ...
    falseAlarmAttack(){
        this.trueTransponderCode = this.transponderCode;
        var emergency = ["7500", "7600", "7700"];
        var fakeSquawk = emergency[Math.floor(Math.random()*3)];
        this.transponderCode = fakeSquawk;
    }

    resetTrueTransponderCode(){
        this.transponderCode = this.trueTransponderCode;
    }


    falseAircraftIdAttack(){
        this.trueId = this.id;
        var n = Math.floor(Math.random() * window.aircraftController.aircraft.list.length);
        this.id = window.aircraftController.aircraft.list[n].id;
    }
